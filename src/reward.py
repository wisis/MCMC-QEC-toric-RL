...
def reward(current_state,chain,burn,p):

    eq_count=np.zeros(16)

    size=chain.toric.system_size
    toric_init = copy.deepcopy(chain.toric)
    ...
    for k in range(steps):
         ...
         if k>=burn:
             ...
             eq_count[eq_last]=eq_count[eq_last]+1

    
    sump=np.sum(eq_count)
    sugg_class=define_equivalence_class(chain.toric.qubit_matrix)
        
   #Uncoment if you wanna print the distribution between eq. classes
    #for i in range(16):
        #print("Class: " + str(i))
        #print(eq_count[i]/sump)


    return eq_count[sugg_class]/sump